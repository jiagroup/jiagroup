...
public class BbsTopicMngImpl implements BbsTopicMng {
	...
	public BbsTopic postTopic(Integer userId, Integer siteId, Integer forumId,
			...
			List<String> code) {
		/**
		 * 过滤数据：title，content一致时，不做处理。
		 */
		BbsPost bbsPost = bbsPostMng.findByTitleAndContent(title, content);
		
		if(bbsPost != null){
			return bbsPost.getTopic();
		}
		bbsPost = bbsPostMng.findByTitle(title);
		...
		BbsTopic topic = null;
		if(bbsPost != null){
			topic = bbsPost.getTopic();
		}
		else{
			topic = createTopic(category);
		}
	...
	public Pagination getListByForumIdAndKeyword(Integer forumId, int pageNo, int pageSize, String keyword) {
		return dao.getListByForumIdAndKeyword(forumId, pageNo, pageSize, keyword);
	}
	...
	public List<BbsTopic> getTodayHotPost(int count) {
		return dao.getTodayHotPost(count);
	}
}
