...
public class TopicDayPageDirective extends AbstractTopicPageDirective {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars,
		...
		Integer pageNo = DirectiveUtils.getInt(FrontUtils.PAGE_NO, params);
		Integer count = DirectiveUtils.getInt(FrontUtils.COUNT, params);
		if(pageNo == null){
			pageNo = FrontUtils.getPageNo(env);
		}
		if(count == null){
			count = FrontUtils.getCount(params);
		}
		Pagination page = bbsTopicMng.getForSearchDate(site.getId(),
				getForumId(params), null, day, pageNo, count);
