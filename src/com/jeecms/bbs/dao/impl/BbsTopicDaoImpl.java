...
public class BbsTopicDaoImpl extends HibernateBaseDao<BbsTopic, Integer>
	...
	public Pagination getTopicByTime(Integer siteId, int pageNo, int pageSize) {
		...
		String sql = "SELECT bean.* FROM Bbs_Post bean WHERE bean.create_Time IN (SELECT MAX(post.create_Time) createTime FROM Bbs_Post post WHERE post.site_id = "+siteId+" GROUP BY post.topic_id) ORDER BY bean.create_Time DESC limit " +(pageNo-1) +"," + pageSize;
		SQLQuery query = this.sessionFactory.getCurrentSession().createSQLQuery(sql);
		query.addEntity(BbsPost.class);
	...
	public List<BbsTopic> getListByForumId(Integer forumId) {
		String hql = "from BbsTopic bean where 1=1 and bean.forum.id=:forumId ";
	...
	/*************************************今日最热******************************************/
	public List<BbsTopic> getTodayHotPost(Integer count) {
		String hql = "from BbsTopic bean order by view_count desc";
		return getSession().createQuery(hql).setFirstResult(0).setMaxResults(count).setCacheable(false).list();
	}
	
	@Override
	public Pagination getListByForumIdAndKeyword(Integer forumId, int pageNo, int pageSize, String keyword) {
		String hql = "from BbsTopic bean where 1=1 ";
		if(StringUtils.isNotEmpty(keyword)){
			hql += "and bean.title like '%" + keyword + "%' ";
		}
		Finder f = Finder.create(hql);
		if(forumId != null){
			f = Finder.create(hql + " and bean.forum.id=:forumId ");
			f.setParam("forumId", forumId);
		}
		f.append(" order by bean.sortTime desc");
		return find(f, pageNo, pageSize);
