...
public class BbsPostDaoImpl extends HibernateBaseDao<BbsPost, Integer>
	...
	@Override
	public BbsPost findByTitleAndContent(String title, String content) {
		String hql = "from BbsPost bean where bean.postText.title=? and bean.postText.content=?";
		return (BbsPost) findUnique(hql, title, content);
	}

	@Override
	public BbsPost findByTitle(String title) {
		String hql = "from BbsPost bean where bean.postText.title=?";
		return (BbsPost) findUnique(hql, title);
	}
}